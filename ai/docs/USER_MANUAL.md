...
`ai/docs/guides/setup.md`를 참조하세요.
...
4. 실행 (산출물 작성)
5. 검증 (bash ai/scripts/verify.sh)
...
자유 텍스트 요구사항 → REQUEST.md → REQUEST 리뷰 → spec → plan → spec/plan 리뷰.
...
사용자가 small-task로 지정한 작업을 바로 실행합니다.
REQUEST 정리 → 실행 → 검증 → CURRENT_TASK.md 갱신.
...
```
ai/workspace/handoffs/review-{세션명}/
...
5. 결과를 `ai/workspace/reports/verifications/`에 저장

검증 프롬프트를 프로젝트에 맞게 커스터마이징하는 것이 중요합니다.
기본 프롬프트: `ai/docs/prompts/verify/default.md`
...
| 검증이 계속 FAIL | `ai/docs/prompts/verify/default.md` 검증 기준이 너무 엄격한지 확인 |
| 리뷰가 20턴 넘어도 끝나지 않음 | `awaiting-user`로 전환됨 — 직접 판단 후 진행 지시 |
| 산출물 파일을 못 찾음 | CURRENT_TASK.md의 `Output Files` 섹션 확인 |
| skill이 원하는 출력을 안 냄 | `ai/docs/prompts/recovery/` 참조 |
| Claude CLI verify.sh 실행 실패 | Claude Code CLI 설치 상태 확인 |
| PROJECT_CONTEXT.md가 비어 있음 | `ai/docs/guides/setup.md` 참조하여 채우기 |
