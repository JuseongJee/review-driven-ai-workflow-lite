...
예:
  ...
  bash ai/scripts/prepare_review_pipeline.sh spec-plan ai/workspace/specs/changes/2026-03-12-campaign-plan-change-spec.md ai/workspace/plans/2026-03-12-campaign-plan-plan.md
  bash ai/scripts/prepare_review_pipeline.sh output
EOF
}
...
case "$review_kind" in
  project-context|project-context-review)
    ...
    review_goal="\`ai/docs/prompts/review/project_context_review.md\` 기준으로 품질 기준, 대상, 제약 조건 등 프로젝트 규칙이 실행 입력으로 충분한지 점검"
    ;;
  request|request-review)
    review_type="request-review"
    session_slug="request-review"
    review_target="REQUEST.md"
    review_goal="\`ai/docs/prompts/review/request_review.md\` 기준으로 제약, 완료 조건, 산출물 유형, 위험 요소, 영향 범위 누락이 없는지 점검"
    ;;
...
${plan_path}"
    review_goal="\`ai/docs/prompts/review/spec_review.md\` 기준으로 과도한 설계, 빠진 엣지 케이스, 더 단순한 대안, 품질 검증 전략 누락, 대상 적합성을 점검"
    ;;
  output|output-review|final-output)
    review_type="output-review"
    session_slug="final-output-review"
    review_target="CURRENT_TASK.md"
    review_goal="\`ai/docs/prompts/review/output_review.md\` 기준으로 Acceptance Criteria 충족, 품질 기준 부합, 논리적 일관성, 제약 준수, 과잉 표현 여부를 점검"
    ;;
  *)
