...
TMP="$(mktemp -d)" || { echo "test_review_prompt_inline.sh: 임시 디렉터리 생성 실패 (mktemp rc≠0, TMPDIR='${TMPDIR:-}')" >&2; exit 1; }
[[ -n "$TMP" && -d "$TMP" ]] || { echo "test_review_prompt_inline.sh: 임시 디렉터리 경로 검증 실패 (TMPDIR='${TMPDIR:-}')" >&2; exit 1; }
cleanup() { rm -rf "$TMP"; }
trap cleanup EXIT
