...
`FR 자동 등록 → [small 판단] → REQUEST 정리 → 실행 → 검증 → 필요 시 final diff review → REQUEST 아카이브`

### 큰 작업 / 기존 산출물의 중간 이상 변경

`FR 자동 등록 → [large 판단] → REQUEST 정리 → REQUEST review → spec/change spec → plan → spec/plan review → 실행 → 검증 → final diff review → REQUEST 아카이브`

## Spec 유형 선택

- **새 산출물 spec** (`rd-workflow-workspace/specs/base/`): 처음 만드는 산출물. 목적, 대상, 구조, 품질 기준을 정의.
- **기존 산출물 변경 spec** (`rd-workflow-workspace/specs/changes/`): 이미 있는 산출물을 고치거나 확장. 현재 상태, 변경 이유, 변경 범위를 정의.
...
2. 검증 스크립트(`test.sh`, `lint.sh`, `typecheck.sh`)를 프로젝트에 맞게 설정합니다
3. 빈칸이나 불명확한 제약이 남아 있으면 `PROJECT_CONTEXT` review를 돌립니다

초기 설정 절차는 `rd-workflow/docs/guides/setup.md`에 있습니다.
...
- 검증: `rd-workflow/scripts/test.sh`, `lint.sh`, `typecheck.sh`
